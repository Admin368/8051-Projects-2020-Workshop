...
uchar Xclock = 0;
char Xclock_old;
char XgameSpeed,Xclock_divider;
...
void getinput(){
	//while(key1==0||key2==0||key3==0||key4==0){
		if(key1==0){//bottom racket push_left
			...
			if(bottomracket_location>=6){bottomracket_location=6;}
			//break;
		}
		if(key2==0){//bottom racket push_right
			bottomracket_location++;
			if(bottomracket_location<=-1){bottomracket_location=-1;}
			if(bottomracket_location>=8){bottomracket_location=8;}
			//break;
		}
		if(key3==0){//top racket push_left
			topracket_location++;
			if(topracket_location<=57){topracket_location=57;}
			if(topracket_location>=64){topracket_location=64;}
		}
		if(key4==0){//top racket push_right
			topracket_location--;
			if(topracket_location<=57){topracket_location=57;}
			if(topracket_location>=64){topracket_location=64;}
		}
...
void update_bottomracket(){
	uchar limit=3;
	if(bottomracket_location==0){//racket out of  bound on left
		draw(1);draw(2);
		return;
	}
	if(bottomracket_location==-1){//racket out of  bound on left
		draw(1);
		return;
	}
	if(bottomracket_location==7){//racket out of  bound on right
		draw(8);draw(7);
		return;
	}
	if(bottomracket_location==8){//racket out of  bound on right
		draw(8);
		return;
	}
	for(xx=0;xx<3;xx++){
...
void update_ball(){
	...
	for(xx=0;xx<=5;xx++){// bouncing left wall
		if(ball_location==(9+(8*xx))){
			if(forward==1){increment=9;}
			else if(forward==0){increment=-9;}
			ball_location=ball_location+increment;
			return;
		}
	}
	for(xx=0;xx<=5;xx++){// bouncing left wall
		if(ball_location==(16+(8*xx))){
			if(forward==1){increment=7;}
			else if(forward==0){increment=-7;}
			ball_location=ball_location+increment;
			return;
		}
	}	
	...
	if(ball_location==(bottomracket_location)||ball_location==(bottomracket_location+1)||ball_location==(bottomracket_location+2)){//bottomracket
	//if(ball_location<9){
		...
		forward=1;
		ball_location=ball_location+increment;
		//if(autospeed==1){if(XgameSpeed<highspeed){XgameSpeed++;}}
		}
	if(ball_location==(topracket_location)){increment=7;}
	else if(ball_location==(topracket_location+1)){increment=8;}
	else if(ball_location==(topracket_location+2)){increment=9;}
	if(ball_location==(topracket_location)||ball_location==(topracket_location+1)||ball_location==(topracket_location+2)){//topracket
		ball_location=old_ball_location;
		increment= -(increment);
		forward=0;
		ball_location=ball_location+increment;
	...
	if(ball_location<9){error=10;return;}//lost
	if(ball_location>56){error=20;return;}//won
