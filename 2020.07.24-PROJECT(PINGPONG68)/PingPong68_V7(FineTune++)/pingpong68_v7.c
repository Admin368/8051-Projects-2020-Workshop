...
char p1_score=0; //player1 score
char p2_score=0; //player2 score
//MOVEMENT
char move_method=2;
char rand_char;
char rand_int;
...
//implement score
char lock = 0; //locks the game in error mode until button press
...
void show_score(){
	char sx;
	if(p1_score>4 || p2_score>4){p1_score=0;p2_score=0;}
	for(sx=0;sx<=p1_score;sx++){
		xled(sx); //show player1 score
	}
	for(sx=8;sx>=9-p2_score;sx--){
		xled(sx);//show player2 score
	}
}
void reset(){
	ball_location=12;
	increment=8;
	forward=1;
	bottomracket_location=3;
	topracket_location=59;
	XgameSpeed=17;
	error=0;
	lock=0;
}
...
void check_reset(){
	if(key1==0){reset();}
	else if(key2==0){reset();}
	else if(key3==0){reset();}
	else if(key4==0){reset();}
	//main();
}

char random_num(){//generates random number to create variation for ai moves
	rand_int=rand()%3;
	switch(rand_int){
		case 0:
			rand_char = 0;
